  ...
  const handleStartSignin = useCallback(async (payload) => {
    ...
    try {
      ...
      if (provider === 'kakao') {
        ...
        try {
          // 카톡 앱 설치 여부 확인
          const available = await KakaoLogins.isKakaoTalkLoginAvailable();
          console.log('[KAKAO] talkAvailable =', available, 'platform=', Platform.OS);

          // 로그인 시도
          const res = available
            ? await KakaoLogins.login()                  // ✅ 카톡 앱 SSO
            : await KakaoLogins.loginWithKakaoAccount(); // 🌐 웹 계정 로그인

          console.log('[KAKAO] login result', res);
          ...
          safeSend('SIGNIN_RESULT', {
            ...
            expires_at: Date.now() + 6 * 3600 * 1000, // 6시간 후 만료 기준
          });
        } catch (err) {
